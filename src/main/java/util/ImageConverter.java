...
public class ImageConverter {
    ...
    public static BufferedImage getSprite(BufferedImage sheet, int imageNumber, int size) {
        ...
        if (x >= spritesPerRow || y >= spritesPerColumn) {
            throw new IllegalArgumentException("Sprite number " + imageNumber + " is out of bounds for sheet size " + sheetWidth + "x" + sheetHeight + ". Sheet can only hold " + (spritesPerRow * spritesPerColumn) + " sprites.");
        }
