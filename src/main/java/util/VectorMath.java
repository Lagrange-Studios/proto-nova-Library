...
public class VectorMath {
  ...
  /** Gets the distance between two vectors using distance formula sqrt((x1-x2)^2 + (y1-y2)^2) */
  ...
  /** Gets the magnitude from the vector Also known as the distance from 0,0 sqrt(x^2+y^2) */
  public static double magnitude(Vector position) {
    return Math.sqrt(Math.pow(position.getX(), 2) + Math.pow(position.getY(), 2));
  }

  /**
   * Returns the given vector as a unit vector with a magnitude of 1 x = x / magnitude y = y /
   * magnitude
   */
  ...
  /** Returns the dot product of two vectors x1*x2+y1*y2 */
  ...
  public static double findAngle(Vector position1, Vector position2) {
    ...
    return Math.acos(
        (Math.pow(magnitude3, 2) - Math.pow(magnitude2, 2) - Math.pow(magnitude1, 2))
            / (-2 * magnitude1 * magnitude2));
  }
  ...
  /** Returns the distance of two points as a vector x = x1-x2 y = y1-y2 */
  ...
  /** Returns the normal of a vector also known as the same vector but perpendicular x = -y y = x */
  public static Vector normal(Vector position) {
    return Vector.newBuilder().setX(-position.getY()).setY(position.getX()).build();
  }

  /**
   * Gets the distance between two vectors without the sqrt calculation (faster) (x1-x2)^2 +
   * (y1-y2)^2 Use this when comparing distances instead of getting actual distance value
   */
