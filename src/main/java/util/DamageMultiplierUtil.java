...
public class DamageMultiplierUtil {
  ...
  public static DamageMultiplier getTotalDamageMultiplier(
      Entity entity, ArrayList<Entity> modifiers) {
  ...
  private static DamageMultiplier.Builder multiply(
      DamageMultiplier.Builder damage1, DamageMultiplier damage2) {
    ...
    for (FieldDescriptor field : DamageMultiplier.getDescriptor().getFields()) {
      if (field.getJavaType().equals(Descriptors.FieldDescriptor.JavaType.FLOAT)) {
        if (damage1.hasField(field) && damage2.hasField(field))
          damage1.setField(
              field, (float) damage1.getField(field) * (float) damage2.getField(field));
        else if (damage1.hasField(field))
          continue; // do nothing cause its currently in the builder already
