  ...
  public static TraumaState getTraumaStateFromEntity(Entity entity) {
    ...
    if (entityDamage >= entity.getCritHealth()) {
      ...
      if (entityDamage >= entity.getMaxHealth()) {
        return TraumaState.DEAD;
      } else if (healthDiff - TraumaState.NEAR_DEATH.getTraumaPercentAsDecimal()
          <= damageAfterCrit) {
      ...
      if (entityDamage
          >= entity.getCritHealth() * TraumaState.MORTALLY_WOUNDED.getTraumaPercentAsDecimal()) {
        return TraumaState.MORTALLY_WOUNDED;
      } else if (entityDamage
          >= entity.getCritHealth() * TraumaState.SEVERELY_INJURED.getTraumaPercentAsDecimal()) {
        return TraumaState.SEVERELY_INJURED;
      } else if (entityDamage
          >= entity.getCritHealth() * TraumaState.INJURED.getTraumaPercentAsDecimal()) {
        return TraumaState.INJURED;
      } else if (entityDamage
          >= entity.getCritHealth() * TraumaState.MINOR_INJURIES.getTraumaPercentAsDecimal()) {
