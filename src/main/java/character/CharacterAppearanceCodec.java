...
/**
 * Backward-compatible wire encoding for cosmetic character appearances. The value travels as a
 * namespaced entity tag, so older protocol builds safely preserve or ignore it without granting
 * clients control of gameplay tags.
 */
public final class CharacterAppearanceCodec {
  ...
  public static String encode(
      int skinRgb, int hairStyle, int hairRgb, int underpantsRgb, Set<Integer> scars) {
    if (hairStyle < 0 || hairStyle > 3) throw new IllegalArgumentException("Invalid hair style.");
    StringBuilder result =
        new StringBuilder(PREFIX)
            .append(rgbHex(skinRgb))
            .append(':')
            .append(hairStyle)
            .append(':')
            .append(rgbHex(hairRgb))
            .append(':')
            .append(rgbHex(underpantsRgb))
            .append(':');
    boolean first = true;
    if (scars != null) {
      for (Integer scar : new java.util.TreeSet<>(scars)) {
        if (scar == null || scar < 1 || scar > 10)
          throw new IllegalArgumentException("Invalid scar style.");
    ...
    if (result.length() > MAX_ENCODED_LENGTH)
      throw new IllegalArgumentException("Appearance is too large.");
  ...
  public static final class Appearance {
    ...
    private Appearance(
        int skinRgb, int hairStyle, int hairRgb, int underpantsRgb, List<Integer> scars) {
    ...
    public int getUnderpantsRgb() {
      return underpantsRgb;
    }

    public List<Integer> getScars() {
      return scars;
    }
  }
}
