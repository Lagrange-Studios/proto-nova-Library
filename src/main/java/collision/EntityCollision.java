...
public final class EntityCollision {
  ...
  /**
   * Returns true only when the two hitboxes have positive overlapping area. Merely touching edges
   * is allowed, which prevents entities from sticking after collision resolution places them
  ...
  public static boolean checkCollision(Entity first, Entity second) {
    ...
    if (!isPositiveFinite(firstWidth)
        || !isPositiveFinite(firstHeight)
        || !isPositiveFinite(secondWidth)
        || !isPositiveFinite(secondHeight)) return false;
